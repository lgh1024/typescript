/*
* readonly修饰符：首先是一个关键字，对类中的属性成员进行修饰，修饰后，该属性成员就不能在外面被随意的修改了
* 构造函数中，可以对只读的属性成员的数据进行修改
* 如果构造函数中没有任何的参数，类中的属性成员此时已经使用readonly进行修饰了，那么外部也是不能对这个属性值进行更改的
* 构造函数中的参数可以使用readonly进行修饰。一旦修饰了，那么该类中就有了这个只读的成员属性了，外部可以访问但不能修改
* 构造函数中的参数可以使用public、private、protected进行修饰，无论是哪个进行修饰的，该类中都会自动的添加这么一个属性成员
* */
(() => {
  // readonly 修饰类的成员属性操作
  // 定义一个类型
...
*/
  ...
  class Person {
    ...
    // 构造函数中的name参数，一旦使用public进行修饰后，那么Person中就有了一个公共的name的属性成员
    constructor(public name: string = '大甜甜') {
      this.name = name
    }
    // 构造函数中的name参数，一旦使用private进行修饰后，那么Person中就有了一个私有的name的属性成员
    /*constructor(private name: string = '大甜甜') {
      this.name = name
    }*/
    // 构造函数中的name参数，一旦使用protected进行修饰后，那么Person中就有了一个受保护的name的属性成员(只能在本类和派生类中访问及使用)
